...
router.post(
  ...
  [
    ...
    body("stock", "Enter a stock  ").isLength({ min: 3 }),
    ...
    body("pan", "Enter a pan code ").isLength({ min: 5 }),
...
router.put("/updatebuyer/:id",authuser , async (req, res) => {
  const { ownerName,email,phone,companyName,productName,stock,address,country,state,zip,pan } = req.body;
  ...
  try {
      ...
      if (productName) { newbuyer.productName = productName };
      if (stock) { newbuyer.stock = stock };
